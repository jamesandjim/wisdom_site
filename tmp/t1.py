...
x = dll.JBNV_OpenServer(b'192.168.0.201', 8200, b'admin', b'admin', byref(hwnd))
if x:
    c = dll.JBNV_GetErrorMessage(x, byref(msg), 256, 0x00000804)
    print(x)
    print(c)
    print(msg.value.decode('gbk'))
else:
    r = dll.JBNV_GetFaceImage(b"ymg.jpg", b"out.jpg", byref(pInputImage), byref(SrcFacePos), byref(DstFacePos))
